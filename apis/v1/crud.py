...
bp = Blueprint(__name__.split('.')[-1], __name__, url_prefix=f'/api/{version}/crud')
...
def get_target_list(target, **kwargs):
    ...
    if fields is not None and not isinstance(fields, list):
        # 避免只查一列参数类型错误
        fields = [fields]
    return response(ResponseEnum.OK, paginate_query(OLTPModelsDict[target], fields))
...
@api_wrapper(params={'*target_id': None})
@target_verify_wrapper
def delete_target(target, **kwargs):
    """
    泛化删除资源
    """
    return orm_delete(OLTPModelsDict[target], kwargs['target_id'])
